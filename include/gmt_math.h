...
/*--------------------------------------------------------------------
 *    The GMT-system:	@(#)gmt_math.h	3.3  11/09/99
 *
...
#if HAVE_ACOSH == 0
#define acosh(x) d_log((x) + (d_sqrt(((x)*(x)) - 1.0)))
#else
extern double acosh(double x);
#endif

#if HAVE_ASINH == 0
#define asinh(x) d_log((x) + (d_sqrt(((x)*(x)) + 1.0)))
#else
extern double asinh(double x);
#endif

#if HAVE_ATANH == 0
#define atanh(x) (fabs(x) < 1.0 ? (0.5*d_log((1.0+(x))/(1.0-(x)))) : \
	(0.5*d_log(((x)+1.0)/((x)-1.0))))
#else
